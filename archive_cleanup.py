...
def analyze(*args):
    ...
    for page in gen:
        ...
        try:
            ...
            text = before.replace("{{SWArchive|subdomain=cargobay|url=webapps/cargobay/", "{{CargoBay|url=").replace("AMGweb", "AtomicMassGames")
            ...
            text = re.sub(r"(\{\{Getty.*?url=thewaltdisney.*?)\|author=.*?(\|[a-z]+=.*?)}}", "\\1\\2}}", text)
            text = re.sub(r"(\{\{Getty.*?)\|author=\[\[The Walt.*?]](\|[a-z]+=.*?)}}", "\\1\\2}}", text)
            text = re.sub(r"(\{\{Getty.*?)\[\[Wikipedia:Getty.*?]]", "\\1", text)
            text = re.sub(r"(\{\{Getty\|.*?)\|archivedate=[0-9+]-[0-9]+-[0-9]+(\|.*?)?}}", "\\1\\2}}", text)
            ...
            changed = True
            while "|archive" in text and changed:
                new_text, archive_data = clean_archive_usages(page, text, archive_data, redo=redo)
