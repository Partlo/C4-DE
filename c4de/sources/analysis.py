...
from c4de.sources.engine import AUDIOBOOK_MAPPING, MANGA, LIST_AT_START, LIST_AT_END, EXPANSION
...
DO_NOT_MOVE = ["GEAttr", "DatapadCite", "Star Wars: Datapad (Galactic Starcruiser)"]
...
def augment_appearances(new_apps: SectionItemIds, appearances: FullListData, collapse_audiobooks: bool, index=False):
    ...
    for i, a in enumerate(new_apps.found):
        ...
        for b in audiobooks:
            ...
            elif "(audio drama)" in b.target or (
...
def prepare_reprints(appearances: FullListData, sources: FullListData, items: List[ItemId]):
    ...
    for i in items:
        if i.master.target:
            if i.master.target in appearances.reprints:
                reprints[i.master.target] = appearances.reprints[i.master.target]
            elif i.master.target in sources.reprints:
                reprints[i.master.target] = sources.reprints[i.master.target]
            elif i.master.issue:
                z = f"{i.master.target.replace(' (department)', '')}|{i.master.issue}"
                if z in sources.reprints:
                    reprints[z] = sources.reprints[z]
...
TEMP_REMAP = {
    "Empire Day (episode)": "Empire Day (comic story)",
    "Gathering Forces": "Gathering Forces (comic story)",
    "Path of the Jedi": "Path of the Jedi (comic story)",
    "Vision of Hope": "Vision of Hope (comic story)",
    "Call to Action": "Call to Action (comic story)",
...
def expand_listing(target: str, o: Item, data: FullListData, expanded: int):
    x = data.target[target][0].copy()
    x.index = o.index
    x.extra = o.extra
    if expanded:
...
def build_item_ids_for_section(page: Page, real, media, name, original: List[Item], data: FullListData, other: FullListData,
    ...
    for i, o in enumerate(original):
        o.index = i
        if o.template == "OfficialSite":
            links.append(o)
            continue

        d = determine_id_for_item(o, page, data.unique, data.urls, data.target, other.unique, other.urls, other.target, remap, canon, log)
        if not d and o.parent:
            p = Page(page.site, o.parent)
            if "[[w:c:" in o.original.lower() or "[[:w:c:" in o.original.lower():
                continue
            if is_redirect(p, o.parent):
                if log:
        ...
        if d and d.master.target and d.master.target in EXPANSION:
            to_check = {d.master.target: EXPANSION[d.master.target]}
        elif d and d.master.target and d.master.target.replace(" (trade paperback)", "") in MANGA:
            to_check = MANGA[d.master.target.replace(" (trade paperback)", "")]
        ...
        if to_check:
            ...
            expanded, existing = 0, 0
            for vol, parts in to_check.items():
                if any(p in already for p in parts):
                    existing += 1
                else:
                    for p in parts:
                        if p in page_links:
                            any_expanded.append(expand_listing(p, o, data, expanded))
                            expanded += 1
            if expanded or existing:
                print(f"Expanded series/arc listing of {d.master.target} to {expanded} new / {existing} existing")
                continue
            else:
        ...
        if d and (o.is_card_or_mini() or o.template == FC):
            ...
            if not (not real and o.template == "SW" and o.url and o.url.startswith("films/")):
                links.append(o)
            ...
            if not (d.master.collection_type or d.master.master_page == "Appearances/Collections") or d.current.unknown:
                ...
                if infobox == "MagazineDepartment" and not px:
                    pass
                elif infobox not in VALID_COLLECTIONS and not any(page.title() == i.target for i in px) and not page.title().startswith(d.master.target):
        ...
        elif d and not real and d.master.non_canon and not name.startswith("Non-canon") and not o.canon_override \
                and d.master.target != "Star Tours: The Adventures Continue" and not page.title().endswith("/LEGO"):
            non_canon.append(d)
        elif d and not real and not d.master.non_canon and name.startswith("Non-canon") and not o.canon_override and not \
                ("cameo" in d.current.extra or "{{C|" in d.current.extra or d.current.template == "TFU" or d.current.target == "Star Wars: The Force Unleashed II"):
            # move listings out of NCA/NCS and tag them with the Nc/Ncm templates
            ...
            links.append(o)
        elif d and d.master.master_page in ["Web/Target", "Web/Publisher"]:
            if not (not real and o.template == "SW" and o.url and o.url.startswith("films/")):
                links.append(o)
        elif d:
            found.append(d)
            ...
            if o.is_appearance and o.target and o.target not in checked:
                ...
                if not o.target.lower().startswith("w:c:") and p.exists() and not p.isRedirectPage():
                    ...
                    if "Category:Media that should be listed in Appearances" not in cats:
                        if log:
...
def handle_card_item(d: ItemId, o: Item, cards: Dict[str, List[ItemId]], found: List[ItemId], wrong: List[ItemId],
    ...
    if parent_set == "Topps Star Wars Living Set" or o.template == "ToppsLivingSet":
        d.current.unknown = True
        n = None
        ...
        if n:
            ...
            d.current.override_date = date.strftime("%Y-%m-%d")
...
def determine_if_wrong(page: Page, o: Item, d: ItemId, real: bool, media: bool):
    if media:
        if d:
            if d.master.master_page in ["Web/Target", "Web/Publisher", "Web/Repost"]:
...
def build_new_external_links(page: Page, results: PageComponents, original: List[Item], data: FullListData,
    ...
    for i, o in enumerate(original):
        ...
        if wrong_or_publisher == WRONG:
            wrong.append(d if d else ItemId(o, o, True, False))
            continue
        elif wrong_or_publisher:
            o.publisher_listing = True
            has_publisher = wrong_or_publisher == HAS_PUBLISHER
...
def build_new_section(title, name, section: SectionItemIds, results: PageComponents, mode: str, include_date: bool,
    ...
    for o in found:
        if mode == BY_DATE and o.current.index is None and o.master.index is None:
            if not (o.current.target and "(audio drama)" not in o.current.target):
        ...
        d = build_date_text(o, include_date).replace("E -->", " -->")
        itext = None
        if o.current.template == "ToppsLivingSet":
            print(o.current.unique_id() in section.group_ids)
        ...
        if o.current.unique_id() in section.group_ids:
            ...
            itext = build_item_text(ItemId(o.current, o.master.original_printing, False),
                                    d, sl, final_without_extra, final_items, section.name,
            ...
            itext = build_item_text(o, d, sl, final_without_extra, final_items, section.name,
                                    collapse_audiobooks, section.mark_as_non_canon, results.unlicensed, log)
        if itext:
...
def build_flags(o: ItemId, sl, section_name, is_file=False):
    if is_file and "[[File:" in o.current.original:
        ...
        return ""
    elif is_extra(o, o.current.extra or ''):
        if o.master.future or is_file or o.master.has_content or "Collect" in section_name:
            return ""
        elif "{{c|cut}}" in (o.current.extra or "").lower():
            return ""
        elif o.master.collection_type and "{{crawl}}" in (o.current.extra or "").lower():
            return ""
        else:
...
def build_item_text(o: ItemId, d: str, sl: str, final_without_extra: list, final_items: List[ItemId],
                    section_name, collapse_audiobooks: bool, nc: str, unlicensed: bool,
    ...
    if o.current.bold:
        zt = f"'''{zt}'''"
    elif "audiobook" in o.master.original and is_extra(o, o.current.extra or ''):
        return None
...
def build_date_text(o: ItemId, include_date):
    if o.current.override_date:
        d = o.current.override_date
    elif o.current.original_date and (o.current.unknown or not o.master.has_date()):
        d = f"{o.current.original_date}?"
    elif not o.master.has_date():
        d = "Unknown"
    elif include_date and o.master.date != "Current":
        d = o.master.date
    elif o.master.has_date() or o.master.date == 'Future' or o.master.date == 'Canceled':
        d = ''
    else:
        d = "Unknown"
    return _build_date_text(o.current.canon_override, o.current.override, d)


def _build_date_text(*d):
    x = ': '.join(i for i in d if i and isinstance(i, str))
...
def should_group(section, mode, o, canon):
    if mode == BY_INDEX and section.is_appearances:
        ...
        if o.current.index is None and "audiobook" not in o.master.original and "(audio drama)" not in o.master.original:
            print(f"No index? {o.current.original}, {o.master.original}")
        return False
    return not o.current.original_date
